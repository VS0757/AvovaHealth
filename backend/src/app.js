...
app.post("/upload-pdf", upload.single("pdf"), async (req, res) => {
  try {
    ...
    const prompt =
      "Listed below is a patient's blood work. It will include an array of entries, called 'testsbydate' (could have only one), where entries each include and are split up by a unique 'effectiveDateTime' key associated with a value of the date that the blood test was conducted - only use the date the test was performed, ignore any other dates, such as approval or received - of the form YYYY-MM-DD. It will also include a 'facility' with the facility that the test was conducted at the same level as testsbydate (if can't find, just put string 'None', but it should be the same for each entry). Each entry should contain an array of sub-entries, called 'test' (may only have one) for each and that contains 'bloodtestname' and 'value' (required), that also contains 'range' and 'unit' if it's there, otherwise just set the value to string, 'None'. Your output should be a JSON string only and nothing else" +
      extractedText;
