...
export function getFilteredUnit(test: string) {
  ...
  return foundKey
    ? (formats[foundKey as keyof typeof formats] as RangeItem)
    : null;
}
...
export function getTestRange(
  ...
  if (preconditions.length > 0 && test.MedPreSpecific) {
    const preconditionMatch = preconditions.find(
      (precondition) => test.MedPreSpecific[precondition],
    );
  ...
  let specificTestRange = specificRanges
    ? gender.toLowerCase() === "male"
      ? specificRanges.Male
      : specificRanges.Female
    : gender.toLowerCase() === "male"
      ? test.Male
      : test.Female;
  const entry = specificTestRange.find(
    ({ ageRange }: any) => age < ageRange[1],
  );
